...
namespace IMPression
{
	public class MathFunctions
	{
		...
		{
			return (-(Ln(Constant.Pi) / 2.0)) * z -
				   (Constant.I / 2.0) *
				   (LogGamma(0.25 + (Constant.I * z) / 2.0) - LogGamma(0.25 - (Constant.I * z) / 2.0));
		}

		public static Complex RiemannSiegelZ(Complex z)
		{
			return Exp(Constant.I * RiemannSiegelTheta(z)) * Zeta(Constant.I * z + 0.5);
		}
		...
		{
			return Constant.Pi * radius.Square();
		}

		public static Complex AreaEllipse(Complex r1, Complex r2)
		{
			return Constant.Pi * r1 * r2;
		}
		...
		{
			return (sides * length.Square()) / (4 * Tan(Constant.Pi / sides));
		}

		public static Complex AreaSphere(Complex radius)
		{
			return 4 * Constant.Pi * radius.Square();
		}
		...
		{
			return 2 * Constant.Pi * radius.Square() + 2 * Constant.Pi * radius * height;
		}
		...
		{
			return (4 / 3 * Constant.Pi) * Pow(radius, 3);
		}
		...
		{
			return (4 / 3 * Constant.Pi) * r1 * r2 * r3;
		}
		...
		{
			return (degrees ? RadToDeg(ArcCos(1 / a)) : ArcCos((1 / a)));
		}

		public static Complex ArcCsc(Complex a)
		{
			return (degrees ? RadToDeg(ArcSin(1 / a)) : ArcSin((1 / a)));
		}

		public static Complex ArcCot(Complex a)
		{
			return (degrees ? RadToDeg(Constant.Pi / 2 - ArcTan(a)) : Constant.Pi / 2 - ArcTan(a));
		}

		public static Complex ArcSinh(Complex a)
		{
			return (degrees ? RadToDeg(Ln(a + Sqrt(a * a + 1.0))) : Ln(a + Sqrt(a * a + 1.0)));
		}
		...
		{
			return (Exp(Angle(a) * Constant.I) + Exp(Angle(-a) * Constant.I)) / 2.0;
		}

		public static Complex Sin(Complex a)
		{
			return (Exp(Angle(a) * Constant.I) - Exp(Angle(-a) * Constant.I)) / (2.0 * Constant.I);
		}

		public static Complex Tan(Complex a)
		{
			if (Angle(Abs(a)) == Constant.Pi / 2.0) return Complex.Indeterminate;
			return Sin(a) / Cos(a);
		}
		...
		{
			return value.IsReal() ? Math.Abs(value.Real) : value.Module;
		}

		public static Complex Ln(Complex c)
		{
			if (c.IsReal() && c < 0.0) return Ln(-c) + (Constant.I * Constant.Pi);
			if (c.IsReal()) return Math.Log(c.Real, Constant.E);

			return Math.Log(c.Module, Constant.E) + (Constant.I * c.Argument);
		}
		...
		{
			return Math.Pow(2 / Constant.Pi, (0.25 * (1 - Math.Cos(Constant.Pi * a)))) * Math.Pow(2, (a / 2)) *
				   Gamma(a / 2 + 1);
		}
		...
		{
			if (z > (1 / Constant.E - 0.1)) throw new ParseException("", 0, "Le rayon de convergence est de 1 / e.");
		...
		public static Complex JacobiSymbol(Complex a, Complex n)
		{
			...
			return Pow(a, (n - 1.0) / 2.0) * n.Module;
		}
		...
		public static Complex MobiusMu(Complex n)
		{
			...
			for (double k = 1; k <= n; k++)
			{
				if (GCD(k, n) == 1.0)
				{
					sum += Exp(2.0 * Constant.Pi * Constant.I * (k / n));
				}
			}
			return sum;
		}
		...
		public static Complex RamanujanTau(Complex n)
		{
			var gamma = 5;
			return 0.5 *
				   Integrate(z => Pow(DedekindEta(z), 24) / Exp(2 * Constant.Pi * Constant.I * n * z), Constant.I * gamma - 1,
					   Constant.I * gamma + 1);
		}

		public static Complex RamanujanTauTheta(Complex z)
		{
			return -z * Ln(2 * Constant.Pi) - 0.25 * (Zeta(0, 6 + Constant.I * z) - Zeta(0, 6 - Constant.I * z));
		}
		...
		public static Complex Gamma(Complex x)
		{
			...
			while (x < 0.0)
			{
				if (x == 0.0)
				{
					throw new ParseException($"Gamma({x})", 0, "x doit être supérieur à 0.");
				}
				else if (x > -1.0E-9)
				{
					return (z / ((1.0 + Constant.EulerGamma * x) * x));
				}
				z /= x;
				x += 1.0;
			}
		...
		{
			return SumInf(k => (1.0 / k) - (1.0 / (k + z - 1)), 1) - Constant.EulerGamma;
		}
		...
		{
			return GammaIncomplete(z + 1, -1) / Constant.E;
		}
		...
		{
			return (2.0 / Sqrt(Constant.Pi)) * SumInf(k => Pow(z, 2.0 * k + 1.0) / (Fact(k) * (2.0 * k + 1.0)), 0);
		}
		...
		{
			if(n != Truncate(n)) throw new ParseException(n.ToString(), 0, "n doit être entier");
			return
				Product(
					k =>
						Pow(z - Pow(Constant.E, (2.0 * Constant.Pi * Constant.I * k) / n),
		...
		public static Complex Multinomial(params Complex[] a)
		{
			if (a.Any(x1 => Math.Truncate(x1) != x1))
				throw new ParseException($"Multinomial({a.First(x1 => Math.Truncate(x1) != x1)}; ...)", 0,
					"Tous les paramètres doivent être entiers");
			if (a.Any(x2 => x2 <= 0))
				throw new ParseException($"Multinomial({a.First(x2 => x2 <= 0)}; ...)", 0,
		...
		{
			return Pow(2.0, v / 2.0) * Math.Sqrt(Constant.Pi) * Pow(Constant.E, -(Pow(z, 2.0) / 4.0)) *
		...
		public static Complex FibonacciFunc(Complex nu, Complex z)
		{
			return (Pow(2.0, -nu) * Pow(z + Sqrt(z.Square() + 4.0), nu) -
					Cos(nu * Constant.Pi) * Pow(2.0, nu) * Pow(z + Sqrt(z.Square() + 4.0), -nu)) /
				   Sqrt(z.Square() + 4.0);
		}
		...
		{
			return ((Pow(2, 1 - 2 * l) * Sqrt(Constant.Pi) * Gamma(nu + 2 * l)) / (Fact(nu) * Gamma(l))) *
		...
		{
			return Pow(2.0, nu) * Sqrt(Constant.Pi) *
		...
		{
			return Sqrt(Constant.Pi / 2.0) * (1 / Sqrt(z)) * BesselJ(v + 0.5, z);
		}

		public static Complex SphericalBesselY(Complex v, Complex z)
		{
			return Sqrt(Constant.Pi / 2.0) * (1 / Sqrt(z)) * BesselY(v + 0.5, z);
		}
		...
		public static Complex KelvinBei(Complex v, Complex z)
		{
			...
			return -0.5 * Constant.I * Exp(-0.75 * Constant.I * Constant.Pi * v) * Pow(z, v) * Pow(fs1 * z, -v) *
				   (Exp((3.0 * Constant.I * Constant.Pi * v) / 2.0) * BesselI(v, fs1 * z) - BesselJ(v, fs1 * z));
		}

		public static Complex SphericalHankelH1(Complex nu, Complex z)
		{
			return Sqrt(Constant.Pi / 2.0) * (1.0 / Sqrt(z)) * HankelH1(nu + 0.5, z);
		}

		public static Complex SphericalHankelH2(Complex nu, Complex z)
		{
			return Sqrt(Constant.Pi / 2.0) * (1.0 / Sqrt(z)) * HankelH2(nu + 0.5, z);
		}

		public static Complex SphericalHarmonicYFunc(Complex l, Complex u, Complex v, Complex phi)
		{
			return Sqrt((2.0 * l + 1.0) / (4.0 * Constant.Pi)) * (Sqrt(Gamma(l - u + 1.0)) / Sqrt(Gamma(l + u + 1.0))) *
				   Exp(Constant.I * phi * u) * LegendreP(l, u, 2, Cos(v));
		}

		public static Complex SphericalHarmonicY(Complex n, Complex m, Complex v, Complex phi)
		{
			return Sqrt(((2.0 * n + 1.0) * Fact(n - m)) / (4.0 * Constant.Pi * Fact(n + m))) * Exp(Constant.I * phi * m) *
		...
		public static Complex StieltjesGamma(Complex n)
		{
			...
			return ((Pow(-1, n) * Fact(n)) / (Constant.Pi * 2.0)) *
				   Integrate(x => Exp(-n * Constant.I * x) * Zeta(Exp(Constant.I * x) + 1.0), 0, 2 * Constant.Pi);
		}
		...
		{
			return EllipticF(Constant.Pi / 2, z);
		}
		...
		{
			return (Constant.Pi * (a + b)) / (4 * EllipticK(Pow((a - b) / (a + b), 2)));
		}
		...
		{
			return EllipticPi(n, Constant.Pi / 2, m);
		}
		...
		{
			if (q.Module >= 1)
				throw new ParseException($"EllepticTheta1(z={z}; q={q})", 0, "q doit être entre 0 et 1.");
		...
		{
			if (q.Module >= 1)
				throw new ParseException($"EllepticTheta2(z={z}; q={q})", 0, "q doit être entre 0 et 1.");
		...
		{
			if (q.Module >= 1)
				throw new ParseException($"EllepticTheta3(z={z}; q={q})", 0, "q doit être entre 0 et 1.");
		...
		{
			if (q.Module >= 1)
				throw new ParseException($"EllepticTheta4(z={z}; q={q})", 0, "q doit être entre 0 et 1.");
		...
		public static Complex DedekindEta(Complex z)
		{
			...
			return Pow(Constant.E, (Constant.Pi * Constant.I * z) / 12.0) *
				   ProductInf(k => 1.0 - Pow(Constant.E, 2.0 * Constant.Pi * Constant.I * k * z), 1);
		}

		public static Complex EllipticNomeQ(Complex m)
		{
			return Exp(-((Constant.Pi * EllipticK(1.0 - m)) / EllipticK(m)));
		}

		public static Complex EllipticThetaPrime1(Complex z, Complex q)
		{
			if (q.Module >= 1)
				throw new ParseException($"EllepticThetaPrime1(z={z}; q={q})", 0,
		...
		{
			if (q.Module >= 1)
				throw new ParseException($"EllepticThetaPrime2(z={z}; q={q})", 0,
					"Le module de q doit être inférieur à 1.");
		...
		{
			if (q.Module >= 1)
				throw new ParseException($"EllepticThetaPrime3(z={z}; q={q})", 0,
					"Le module de q doit être inférieur à 1.");
		...
		{
			if (q.Module >= 1)
				throw new ParseException($"EllepticThetaPrime4(z={z}; q={q})", 0,
					"Le module de q doit être inférieur à 1.");
		...
		public static Complex KleinInvariantJ(Complex z)
		{
			...
			return
				Pow(
					Pow(EllipticTheta2(0, Exp(Constant.Pi * Constant.I * z)), 8) +
					Pow(EllipticTheta3(0, Exp(Constant.Pi * Constant.I * z)), 8) +
					Pow(EllipticTheta4(0, Exp(Constant.Pi * Constant.I * z)), 8), 3) /
				(54.0 *
				 Pow(
					 EllipticTheta2(0, Exp(Constant.Pi * Constant.I * z)) *
					 EllipticTheta3(0, Exp(Constant.Pi * Constant.I * z)) *
					 EllipticTheta4(0, Exp(Constant.Pi * Constant.I * z)), 8));
		}

		public static Complex ModularLambda(Complex z)
		{
			if (z.Imaginary <= 0) return Complex.Indeterminate;
			return 16.0 * Exp(Constant.I * Constant.Pi * z) *
				   ProductInf(
					   k =>
						   Pow(
							   (1.0 + Exp(2.0 * k * Constant.Pi * Constant.I * z)) /
							   (1.0 + Exp((2.0 * k - 1.0) * Constant.Pi * Constant.I * z)), 8), 1);
		}
		...
		private static bool degrees => EquationParser.UseDegrees;
		...
		{
			return degrees ? DegToRad(input) : input;
		}
